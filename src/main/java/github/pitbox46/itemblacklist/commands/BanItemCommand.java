...
public class BanItemCommand {
    public static ArgumentBuilder<CommandSourceStack, ?> register(CommandDispatcher<CommandSourceStack> dispatcher) {
        return Commands
                ...
                .then(Commands.argument("item", ItemArgument.item()).executes(BanItemCommand::banItem)
    ...
    private static int banHand(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        int retVal = removeItem(context.getSource(), new ItemInput(stack.getItem(), stack.getTag()), PermissionLevel.LEVEL_4);
    ...
    private static int banHandWithPermission(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        ServerPlayer player = context.getSource().getPlayerOrException();
        ItemStack stack = player.getMainHandItem();
        int retVal = removeItem(context.getSource(), new ItemInput(stack.getItem(), stack.getTag()), PermissionLevel.getFromInt(permissionLevel));
    ...
    private static int removeItem(CommandSourceStack commandSource, ItemInput input, PermissionLevel permissionLevel) throws CommandSyntaxException {
        ...
        if (!hasPermission) {
            commandSource.sendFailure(new TextComponent("Command source does not have adequate permissions. Must have at least permission level " + permissionLevel.ordinal()));
            return 0;
        }
        ...
        commandSource.sendSuccess(new TextComponent("Item banned: ").append(itemComponent).append(" for players " + permissionLevel + " and under"), true);
